function mse = awgnPerSnrFittingMse(sinrStore,perStore,format,mcs,channelCoding,dataLength,beta)

numPkts = size(sinrStore,3);
snrEff = nan(numPkts,1);
for pktIdx = 1: numPkts
    snrEff(pktIdx) = tgaxLinkPerformanceModel.effectiveSINR(sinrStore(:,:,pktIdx),beta);
end
...
lut = tgaxLinkPerformanceModel.selectAWGNLUT(format,mcs,channelCoding,dataLength);
